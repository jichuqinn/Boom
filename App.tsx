...
const App: React.FC = () => {
  ...
  const handleLaneInput = useCallback(
    (laneIndex: number) => {
      ...
      const hittableBeat = beatsRef.current.find(
        (b) =>
          !b.hit &&
          !b.missed &&
          b.lane === laneIndex &&
      ...
      if (hittableBeat) {
        ...
        const laneStart = {
          currentTrackWidth: TRACK_WIDTH_BOTTOM * PERSPECTIVE,
        };
        const laneEnd = { currentTrackWidth: TRACK_WIDTH_BOTTOM };
        const trackW =
          laneStart.currentTrackWidth +
          (laneEnd.currentTrackWidth - laneStart.currentTrackWidth) *
            hitVisualProgress;
    ...
    [gameState]
  ); // Removed combo/score from dependencies!
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      pulseBrightnessRef.current *= 0.9;
      ...
      // Check finish
      if (
        now > audioService.getDuration() + 2 &&
        audioService.getDuration() > 0
      ) {
      ...
      if (canvas) {
        ...
        if (ctx) {
          ...
          const shakeVal =
            shakeIntensityRef.current + (heatVal > 90 ? Math.random() * 2 : 0); // Extra shake at high heat
          ...
          const bounceY =
            pulseVal * 25 + (heatVal > 80 ? Math.sin(now * 20) * 5 : 0);
          ...
          floatingGeoRef.current.forEach((g) => {
            ...
            ctx.translate(gx, gy);
            ctx.rotate(
              g.rotation + (heatVal > 80 ? Math.sin(now * 10) * 0.1 : 0)
            ); // Jitter at high heat
          ...
          for (let i = 0; i < barCount; i++) {
            ...
            const barH =
              hBase * wave * (1 - distFromCenter * 0.5) + pulseVal * 50;
          ...
          const monoHeight =
            monoHeightBase + (heatVal / 100) * h * 0.3 + pulseVal * 100;
          const monoColor =
            tierVal === 1
              ? "#22d3ee"
              : tierVal === 2
          ...
          // 5. Draw Receptors
          const hitVisualProgress =
            (HIT_Y_PERCENT - HORIZON_Y) / (1 - HORIZON_Y);
          for (let i = 0; i < LANES; i++) {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black touch-none select-none font-sans">
      <canvas
        ref={canvasRef}
      ...
      {(gameState === GameState.PLAYING ||
        ...
        <div
          ...
          style={{ top: `${HIT_Y_PERCENT * 100}%`, marginTop: "-150px" }}
        >
          <GameCharacter
            isHit={isHitFrame}
      ...
      {gameState === GameState.PLAYING && currentTimeDisplay < 0 && (
        <div className="absolute inset-0 z-50 flex items-center justify-center flex-col pointer-events-none">
          <div className="text-white/20 font-['Black_Ops_One'] text-6xl tracking-[1em] mb-8 animate-pulse">
            GET READY
          </div>
          ...
          <div className="flex flex-col items-center justify-center transform translate-y-[-50px]">
            <div
              ...
              style={{
                ...
                textShadow:
                  tier > 0 ? `0 0 ${tier * 20}px currentColor` : "none",
            ...
            <div className="text-white/50 text-xl font-bold tracking-[0.5em] mt-[-10px]">
              COMBO
            </div>
            ...
            <div
              className={`w-96 h-3 bg-gray-900 rounded-full mt-6 overflow-hidden border border-white/10 transition-all duration-300 ${
                heat > 80
                  ? "shadow-[0_0_30px_#d946ef] border-white/80 scale-105 animate-pulse"
            ...
            {/* Overdrive Label */}
            <div
              className={`mt-2 text-xs font-black tracking-[0.5em] transition-opacity duration-300 ${
                heat > 80
                  ? "opacity-100 text-fuchsia-400 animate-pulse"
                  : "opacity-0"
              }`}
            >
              OVERDRIVE
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-xl p-4">
          <div className="max-w-md w-full bg-zinc-900/90 p-8 rounded-3xl shadow-[0_0_100px_rgba(6,182,212,0.3)] border border-white/10">
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl text-white/70 font-bold tracking-[0.5em] uppercase">
                光环 · 节奏
              </h2>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-cyan-900/30 to-blue-900/30 p-1 rounded-xl group relative overflow-hidden">
                ...
                <p className="text-center text-cyan-300/50 text-[10px] mt-2 pb-1 font-mono">
                  体验自动生成的合成波三轨谱面
                </p>
              </div>
              ...
              <div className="relative flex items-center py-2">
                ...
                <span className="flex-shrink-0 mx-4 text-zinc-600 text-[10px] font-bold tracking-widest">
                  自定义模式
                </span>
                <div className="flex-grow border-t border-zinc-800"></div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-500 mb-1 tracking-wider">
                    上传音乐文件
                  </label>
                  <input
                    type="file"
                  ...
                  />
                  {isFileLoaded && (
                    <p className="text-cyan-400 text-[10px] mt-1 font-mono ml-1">
                      ✓ 音频已加载
                    </p>
                  )}
                </div>
                ...
                <div>
                  <div className="flex justify-between items-end mb-1">
                    <label className="block text-[10px] font-bold text-zinc-500 tracking-wider">
                      谱面数据 (JSON)
                    </label>
                    <button
                ...
                <button
                  ...
                  disabled={
                    !isFileLoaded && !audioService.isReady() && !rawBeatsInput
                  }
                >
                  开始游戏
                </button>
      ...
      {gameState === GameState.FINISHED && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-xl">
          <div className="text-center animate-bounce-in relative">
            ...
            <div className="text-xl text-white/50 tracking-[1em] mb-8 font-bold">
              演奏完成
            </div>
